...
namespace GSpawn
{
    ...
    {
        [SerializeField]
        [UIFieldConfig("Wire color", "The wire color used to draw the extrude gizmo box.", "Extrude Gizmo", false)]
        private Color _extrudeWireColor = defaultExtrudeWireColor;
        [SerializeField]
        [UIFieldConfig("X axis color", "The color of the X axis handles.")]
        private Color _extrudeXAxisColor = defaultExtrudeXAxisColor;
        [SerializeField]
        [UIFieldConfig("Y axis color", "The color of the Y axis handles.")]
        private Color _extrudeYAxisColor = defaultExtrudeYAxisColor;
        [SerializeField]
        [UIFieldConfig("Z axis color", "The color of the Z axis handles.")]
        private Color _extrudeZAxisColor = defaultExtrudeZAxisColor;
        [SerializeField]
        [UIFieldConfig("Sgl-axis handle size", "The size of the single-axis handles.")]
        [Min(1e-4f)]
        private float _extrudeSglAxisSize = defaultExtrudeSglAxisSize;
        [SerializeField]
        [UIFieldConfig("Extrude cell wire color", "The wire color used to draw the extrude cells.")]
        private Color _extrudeCellWireColor = defaultExtrudeCellWireColor;
        [SerializeField]
        [UIFieldConfig("Extrude cell fill color", "The fill color used to draw the extrude cells.")]
        private Color _extrudeCellFillColor = defaultExtrudeCellFillColor;
        [SerializeField]
        [UIFieldConfig("Dbl-axis tick size", "The size of the double-axis ticks.")]
        [Min(1e-4f)]
        private float _extrudeDblAxisSize = defaultExtrudeDblAxisSize;
        [SerializeField]
        [UIFieldConfig("Show info text", "If checked, the plugin will offer textual information during while using the extrude gizmo.")]
        private bool _extrudeShowInfoText = defaultExtrudeShowInfoText;
        [SerializeField]
        [UIFieldConfig("Select spawned", "If this is checked, all objects which were spawned as a result of the extrusion, will be selected.")]
        private bool _extrudeSelectSpawned = defaultExtrudeSelectSpawned;
        [SerializeField]
        [UIFieldConfig("Plane size", "The mirror plane size.", "Mirror Gizmo", true)]
        [Min(1e-4f)]
        private float _mirrorPlaneSize = defaultMirrorPlaneSize;
        [SerializeField]
        [UIFieldConfig("XY plane color", "The color of the mirror XY plane.")]
        private Color _mirrorXYPlaneColor = defaultMirrorXYPlaneColor;
        [SerializeField]
        [UIFieldConfig("YZ plane color", "The color of the mirror YZ plane.")]
        private Color _mirrorYZPlaneColor = defaultMirrorYZPlaneColor;
        [SerializeField]
        [UIFieldConfig("ZX plane color", "The color of the mirror ZX plane.")]
        private Color _mirrorZXPlaneColor = defaultMirrorZXPlaneColor;
        [SerializeField]
        [UIFieldConfig("Plane border color", "The border color of the mirror planes.")]
        private Color _mirrorPlaneBorderColor = defaultMirrorPlaneBorderColor;
        [SerializeField]
        [UIFieldConfig("Indicator wire color", "The wire color used to draw the mirrored object volumes.")]
        private Color _mirrorIndicatorWireColor = defaultMirrorIndicatorWireColor;
        [SerializeField]
        [UIFieldConfig("Indicator fill color", "The fill color used to draw the mirrored object volumes.")]
        private Color _mirrorIndicatorFillColor = defaultMirrorIndicatorFillColor;
        [SerializeField]
        [UIFieldConfig("Symmetric pair highlight radius", "The radius used to collect symmetric object pairs.")]
        [Min(0.1f)]
        private float _mirrorSymmetricPairHighlightRadius = defaultMirrorSymmetricPairHighlightRadius;
        [SerializeField]
        [UIFieldConfig("Show info text", "If checked, the plugin will offer textual information during while using the mirror gizmo.")]
