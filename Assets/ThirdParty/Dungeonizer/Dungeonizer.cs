    ...
    class Dungeon
    {
        ...
        public void Generate()
        {
            ...
            for (int x = 0; x < map_size_x - 1; x++)
            {
                for (int y = 0; y < map_size_y - 1; y++)
                {
                    if (walls.Contains(map[x, y + 1].type) && walls.Contains(map[x + 1, y].type) && roomsandfloors.Contains(map[x + 1, y + 1].type))
                    { //north
                        map[x, y].type = 4;
                    ...
                    if (y > 0)
                    {
                        if (walls.Contains(map[x + 1, y].type) && walls.Contains(map[x, y - 1].type) && roomsandfloors.Contains(map[x + 1, y - 1].type))
                        { //north
                            map[x, y].type = 5;
                            map[x, y].room = map[x + 1, y - 1].room;
