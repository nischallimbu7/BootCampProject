...
namespace GSpawn
{
    ...
    {
        [SerializeField]
        [UIFieldConfig("Allow child select", "If checked, clicking on the same object will switch between selecting the prefab instance root and the child which was actually clicked on. " +
        ...
        [SerializeField]
        [UIFieldConfig("Parent color", "The highlight color used for parent objects.", "Selection Highlight", true)]
        private Color _parentHighlightColor = defaultParentHighlightColor;
        [SerializeField]
        [UIFieldConfig("Child color", "The highlight color used for child objects.")]
        private Color _childHighlightColor = defaultChildHighlightColor;
        [SerializeField]
        [UIFieldConfig("Opacity", "The highlight opacity.")]
        [Range(0.0f, 1.0f)]
        private float _highlightOpacity = defaultHighlightOpacity;
        [SerializeField]
        [UIFieldConfig("Camera gizmo color", "The color used to draw selected camera gizmos.")]
        private Color _cameraGizmoColor = defaultCameraGizmoColor;

        [SerializeField]
        [UIFieldConfig("Border color", "The selection rectangle border color", "Selection Rectangle", true)]
        private Color _selRectBorderColor = defaultSelRectBorderColor;
        [SerializeField]
        [UIFieldConfig("Fill color", "The selection rectangle fill color")]
        private Color _selRectFillColor = defaultSelRectFillColor;

        [SerializeField]
        [UIFieldConfig("Tick color", "The color of the ticks that represent the segment chain nodes.", "Selection Segments", true)]
        private Color _selSegmentsTickColor = defaultSelSegmentsTickColor;
        [SerializeField]
        [UIFieldConfig("Segment color", "The segment color.")]
        private Color _selSegmentsSegmentColor = defaultSelSegmentsSegmentColor;
        [SerializeField]
        [UIFieldConfig("Tick size", "The size of the ticks that represent the segment chain nodes.")]
        [Min(0.0f)]
        private float _selSegmentsTickSize = defaultSelSegmentsTickSize;

        [SerializeField]
        [UIFieldConfig("Wire color", "The color used to draw the object selection box wire representation.", "Selection Box", true)]
        private Color _selBoxWireColor = defaultSelBoxWireColor;

        [SerializeField]
        [UIFieldConfig("Transform session", "The transform session that should automatically be activated when spawning a prefab in object selection mode via drag and drop.", "Prefab Spawn", true)]
