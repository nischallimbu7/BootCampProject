...
namespace GSpawn
{
    ...
    {
        [SerializeField]
        [UIFieldConfig("Wire color", "The grid wire color (i.e. cell line color).")]
        private Color _wireColor = defaultWireColor;
        [SerializeField]
        [UIFieldConfig("Fill color", "The grid fill color (i.e. cell area color).")]
        private Color _fillColor = defaultFillColor;
        [SerializeField]
        [UIFieldConfig("Draw coordinate system", "If this is checked, the grid will draw its local coordinate system.")]
        private bool _drawCoordSystem = defaultDrawCoordSystem;
        [SerializeField]
        [UIFieldConfig("X axis color", "The color of the grid's local coordinate system X axis.")]
        private Color _xAxisColor = defaultXAxisColor;
        [SerializeField]
        [UIFieldConfig("Y axis color", "The color of the grid's local coordinate system Y axis.")]
        private Color _yAxisColor = defaultYAxisColor;
        [SerializeField]
        [UIFieldConfig("Z axis color", "The color of the grid's local coordinate system Z axis.")]
        private Color _zAxisColor = defaultZAxisColor;
        [SerializeField]
        [UIFieldConfig("Infinite X axis", "Allows you to specify whether the local X axis must be drawn as an infinite line segment.")]
        private bool _infiniteXAxis = defaultInfiniteXAxis;
        [SerializeField]
        [UIFieldConfig("Infinite Y axis", "Allows you to specify whether the local Y axis must be drawn as an infinite line segment.")]
        private bool _infiniteYAxis = defaultInfiniteYAxis;
        [SerializeField]
        [UIFieldConfig("Infinite Z axis", "Allows you to specify whether the local Z axis must be drawn as an infinite line segment.")]
        private bool _infiniteZAxis = defaultInfiniteZAxis;
        [SerializeField]
        [Min(0.0f)]
        [UIFieldConfig("Finite axis length", "This value is used to draw finite grid local axes.")]
