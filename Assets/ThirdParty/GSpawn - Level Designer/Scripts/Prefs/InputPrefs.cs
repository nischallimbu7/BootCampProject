...
namespace GSpawn
{
    ...
    public class InputPrefs : Prefs<InputPrefs>
    {
        ...
        [SerializeField]
        [UIFieldConfig("Keyboard X rotation step", "The amount of rotation applied around the X axis when rotating entities with the keyboard.", "Keyboard", true)]
        [Min(1e-2f)]
        private float _keyboardXRotationStep = defaultKeyboardXRotationStep;
        [SerializeField]
        [UIFieldConfig("Keyboard Y rotation step", "The amount of rotation applied around the Y axis when rotating entities with the keyboard.")]
        [Min(1e-2f)]
        private float _keyboardYRotationStep = defaultKeyboardYRotationStep;
        [SerializeField]
        [UIFieldConfig("Keyboard Z rotation step", "The amount of rotation applied around the Z axis when rotating entities with the keyboard.")]
        [Min(1e-2f)]
        private float _keyboardZRotationStep = defaultKeyboardZRotationStep;
        [SerializeField]
        [UIFieldConfig("Offset sensitivity", "Mouse sensitivity when offsetting entities (e.g. when offsetting objects from a surface).", "Mouse", true)]
        [Range(1e-2f, 10.0f)]
        private float _mouseOffsetSensitivity = defaultMouseOffsetSensitivity;
        [SerializeField]
        [UIFieldConfig("Rotation sensitivity", "Mouse sensitivity when rotating entities.")]
        [Range(1e-2f, 10.0f)]
        private float _mouseRotationSensitivity = defaultMouseRotationSensitivity;
        [SerializeField]
        [UIFieldConfig("Scroll rotation step", "The amount of rotation applied when rotating entities with the mouse scroll wheel.")]
        [Min(1e-2f)]
        private float _scrollRotationStep = defaultScrollRotationStep;
        [SerializeField]
        [UIFieldConfig("Scale sensitivity", "Mouse sensitivity when scaling entities.")]
        [Range(1e-2f, 10.0f)]
        private float _mouseScaleSensitivity = defaultMouseScaleSensitivity;
        [SerializeField]
        [UIFieldConfig("Log shortcuts", "If checked, shortcuts will be displayed on the screen when they are used. Useful when recording video tutorials.", "Shortcut Logging", true)]
        private bool _logShortcuts = defaultLogShortcuts;
        [SerializeField]
        [UIFieldConfig("Log text color", "The text color used when drawing the shortcuts on the screen.")]
        private Color _shortcutLogTextColor = defaultShortcutLogTextColor;
        [SerializeField]
        [UIFieldConfig("Log font size", "The font size used when drawing the shortcuts on the screen.")]
        [Min(10)]
        private int _shortcutLogFontSize = defaultShortcutLogFontSize;
        [SerializeField]
        [UIFieldConfig("Pick spawn guide prefab", "The modifier key used to enable prefab picking when working with the spawn guide.", "Shortcuts", true)]
        private PickSpawnGuidePrefabKeys _pickSpawnGuidePrefabKeys = defaultPickSpawnGuidePrefabKeys;
        [SerializeField]
        [UIFieldConfig("Selection replace", "The modifier key used to enable selection replacement.")]
